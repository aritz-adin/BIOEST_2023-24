...
```{r, include=FALSE}
par(mfrow=c(1,2))
hist(diabetes$bp.1s, xlab="", main="Presión sistólica (mmHg)")
boxplot(diabetes$bp.1s, main="Presión sistólica (mmHg)")
```
...
```{r, include=FALSE}
PASWR2::eda(diabetes$bp.1s)
CV <- sd(diabetes$bp.1s)/mean(diabetes$bp.1s)
CV
```
...
boxplot(diabetes$bp.1s ~ diabetes$gender, ylab="Presión sistólica (mmHg)", xlab="Gender")
histogram(~ bp.1s | gender, xlab="presión sistólica", type="density", data=diabetes)
```
...
```{r, include=FALSE}
plot(diabetes$bp.1s, diabetes$bp.1d, xlab="Presión sistólica", ylab="Presión diastólica")
abline(lm(bp.1d ~ bp.1s, data=diabetes), col="red")

cov(diabetes$bp.1s, diabetes$bp.1d)
cor(diabetes$bp.1s, diabetes$bp.1d)
```
...
# Calculamos los niveles de colesterol mínimos 
x <- min(diabetes.male$chol)
y <- min(diabetes.female$chol)
